resource "azurerm_key_vault" "bulwark-vault" {
...
resource "azurerm_key_vault_secret" "client_certificate" {
  ...
  value        = module.aks_weu_1.client_certificate
  key_vault_id = azurerm_key_vault.bulwark-vault.id
  tags         = var.tags
}
...
resource "azurerm_key_vault_secret" "client_key" {
  ...
  value        = module.aks_weu_1.client_key
  key_vault_id = azurerm_key_vault.bulwark-vault.id
  tags         = var.tags
}
...
resource "azurerm_key_vault_secret" "cluster_ca_certificate" {
  ...
  value        = module.aks_weu_1.cluster_ca_certificate
  key_vault_id = azurerm_key_vault.bulwark-vault.id
  tags         = var.tags
}
...
resource "azurerm_key_vault_secret" "kube_admin_config" {
  ...
  value        = module.aks_weu_1.kube_admin_config_raw
  key_vault_id = azurerm_key_vault.bulwark-vault.id
  content_type = "password"
}
